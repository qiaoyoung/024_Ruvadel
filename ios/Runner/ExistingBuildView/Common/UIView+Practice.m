...
@implementation UIView (Practice)

//: #pragma mark - HitTest
#pragma mark - HitTest
//: - (UIView *)swizzling_hitTest:(CGPoint)point withEvent:(UIEvent *)event
- (UIView *)execute:(CGPoint)point convertYield:(UIEvent *)event
{
    //: UIView *view = [self swizzling_hitTest:point withEvent:event];
    UIView *view = [self execute:point convertYield:event];
    //: if (view) {
    if (view) {
    }
    //: return view;
...
- (void)relativeFar:(CGRect)frame
{
    //: [self swizzling_setFrame:frame];
    [self relativeFar:frame];
