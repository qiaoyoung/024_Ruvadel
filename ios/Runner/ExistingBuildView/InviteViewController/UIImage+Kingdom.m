...
- (UIImage *)audience
{
    ...
    CGFloat pixels = [[KnownHear current] visual];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self grace:pixels];
...
- (UIImage *)quantitativeRelationPixels: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self asSave:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)pair: (CGSize)size
{
...
- (UIImage *)grace: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self quantitativeRelationPixels:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self quantitativeRelationPixels:suggestPixels];
    }
}
