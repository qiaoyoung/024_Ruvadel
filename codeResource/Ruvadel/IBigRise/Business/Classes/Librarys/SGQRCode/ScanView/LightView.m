...
@property (nonatomic, strong) PathConfigure *configure;
...
//: @property (nonatomic, strong) UIView *contentView;
@property (nonatomic, strong) UIView *supernumerary;
...
- (UIView *)contentView {
	...
	[self setLeadJoin:_doubleTapBlock];
        ...
        [self lab:_contentView].backgroundColor = [UIColor clearColor];
        //: _contentView.clipsToBounds = YES;
        _contentView.clipsToBounds = YES;
	[self setLeadJoin:_doubleTapBlock];
    }
    //: return _contentView;
    return [self lab:_contentView];
}
...
- (void)startingWith {
    ...
    [[self lab:self.contentView] addSubview:self.scanlineImgView];
...
- (void)untilCrop {
    ...
    if (self.isSelected) {
        //: self.isSelected = NO;
        self.isSelected = NO;
	[self setSupernumerary:_contentView];
    //: } else {
    } else {
        //: self.isSelected = YES;
        self.isSelected = YES;
    ...
    if ([self need:self.doubleTapBlock]) {
...
+ (instancetype)duringJump:(CGRect)frame visible_strong:(PathConfigure *)configure {
    ...
    return [[LightView alloc] initWithDegree:frame afloatConfigure:configure];
}
...
- (void)surplusage:(CGFloat)borderX opinion:(CGFloat)borderY past:(CGFloat)cornerLength computer:(CGFloat) insideExcess res:(CGFloat)outsideExcess {
...
- (void)setSupernumerary:(UIView *)supernumerary {
    //: OC_CUSTOM_PROPERTY_INJECT
    _supernumerary = supernumerary;
}
...
- (instancetype)initWithDegree:(CGRect)frame afloatConfigure:(PathConfigure *)configure {
	...
	[self setLeadJoin:_doubleTapBlock];
        ...
        [self addSubview:[self lab:self.contentView]];

        //: UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tap_action)];
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(untilCrop)];
...
- (void)setScanFrame:(CGRect)scanFrame {
    ...
    [self lab:self.contentView].frame = scanFrame;
	[self setLeadJoin:_doubleTapBlock];
...
- (void)stickExcess:(CGFloat)borderX boldOnRoot:(CGFloat)borderY titExcess:(CGFloat)borderW playTo:(CGFloat)cornerLength nowDown:(CGFloat) insideExcess alfrescoToday:(CGFloat)outsideExcess {
...
- (void)setLeadJoin:(SGScanViewDoubleTapBlock)leadJoin {
    //: OC_CUSTOM_PROPERTY_INJECT
    _leadJoin = leadJoin;
}

- (UIView *)lab:(UIView *)supernumerary {
    //: OC_CUSTOM_PROPERTY_INJECT
    _supernumerary = supernumerary;
...
- (SGScanViewDoubleTapBlock)need:(SGScanViewDoubleTapBlock)leadJoin {
    //: OC_CUSTOM_PROPERTY_INJECT
    _leadJoin = leadJoin;
    return leadJoin;
}
...
- (void)lemonTo:(CGFloat)borderX scupper:(CGFloat)borderY security:(CGFloat)borderH full:(CGFloat)cornerLength bottom:(CGFloat) insideExcess iconPersonFloat:(CGFloat)outsideExcess {
...
- (void)basic {
    ...
    CGFloat w = [self lab:_contentView].frame.size.width;
...
- (void)indoorsEnable:(CGFloat)borderX on:(CGFloat)borderY array:(CGFloat)borderW present:(CGFloat)borderH rootThumb:(CGFloat)cornerLength related:(CGFloat) insideExcess boundary:(CGFloat)outsideExcess {
...
- (void)drawRect:(CGRect)rect {
    ...
    [self surplusage:borderX opinion:borderY past:cornerLength computer:insideExcess res:outsideExcess];
    ...
    [self lemonTo:borderX scupper:borderY security:borderH full:cornerLength bottom:insideExcess iconPersonFloat:outsideExcess];
    ...
    [self stickExcess:borderX boldOnRoot:borderY titExcess:borderW playTo:cornerLength nowDown:insideExcess alfrescoToday:outsideExcess];
    ...
    [self indoorsEnable:borderX on:borderY array:borderW present:borderH rootThumb:cornerLength related:insideExcess boundary:outsideExcess];
}
...
- (void)arc {
    ...
    CGFloat contentViewHeight = CGRectGetHeight([self lab:self.contentView].frame);
