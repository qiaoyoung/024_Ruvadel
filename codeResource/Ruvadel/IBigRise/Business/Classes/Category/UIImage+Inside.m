...
+ (Byte *)IdentifyDataToByte:(StructIdentifyData *)data {
    for (int i = 0; i < data->incomplete; i++) {
        data->disco[i] ^= data->differ;
    }
    data->disco[data->incomplete] = 0;
    return data->disco;
}

//: tiff
+ (NSString *)moduleRoveError {
    /* static */ NSString *moduleRoveError = nil;
    if (!moduleRoveError) {
		NSArray<NSNumber *> *origin = @[@126, @99, @108, @108, @78];
		NSData *data = [IdentifyData IdentifyDataToData:origin];
        StructIdentifyData value = (StructIdentifyData){10, (Byte *)data.bytes, 4};
        moduleRoveError = [self StringFromIdentifyData:&value];
    }
    return moduleRoveError;
}

//: yyyy-MM-dd HH:mm:ss
+ (NSString *)layoutPreserveKey {
    /* static */ NSString *layoutPreserveKey = nil;
    if (!layoutPreserveKey) {
		NSArray<NSNumber *> *origin = @[@72, @72, @72, @72, @28, @124, @124, @28, @85, @85, @17, @121, @121, @11, @92, @92, @11, @66, @66, @199];
		NSData *data = [IdentifyData IdentifyDataToData:origin];
        StructIdentifyData value = (StructIdentifyData){49, (Byte *)data.bytes, 19};
        layoutPreserveKey = [self StringFromIdentifyData:&value];
    }
    return layoutPreserveKey;
}

+ (NSString *)StringFromIdentifyData:(StructIdentifyData *)data {
    return [NSString stringWithUTF8String:(char *)[self IdentifyDataToByte:data]];
}

//: png
+ (NSString *)widgetTradeFormat {
    /* static */ NSString *widgetTradeFormat = nil;
    if (!widgetTradeFormat) {
		NSArray<NSNumber *> *origin = @[@109, @115, @122, @38];
		NSData *data = [IdentifyData IdentifyDataToData:origin];
        StructIdentifyData value = (StructIdentifyData){29, (Byte *)data.bytes, 3};
        widgetTradeFormat = [self StringFromIdentifyData:&value];
    }
    return widgetTradeFormat;
}

//: jpeg
+ (NSString *)commonWealthFormat {
    /* static */ NSString *commonWealthFormat = nil;
    if (!commonWealthFormat) {
		NSArray<NSNumber *> *origin = @[@188, @166, @179, @177, @193];
		NSData *data = [IdentifyData IdentifyDataToData:origin];
        StructIdentifyData value = (StructIdentifyData){214, (Byte *)data.bytes, 4};
        commonWealthFormat = [self StringFromIdentifyData:&value];
    }
    return commonWealthFormat;
}

//: gif
+ (NSString *)widgetEmotionKey {
    /* static */ NSString *widgetEmotionKey = nil;
    if (!widgetEmotionKey) {
		NSArray<NSNumber *> *origin = @[@81, @95, @80, @85];
		NSData *data = [IdentifyData IdentifyDataToData:origin];
        StructIdentifyData value = (StructIdentifyData){54, (Byte *)data.bytes, 3};
        widgetEmotionKey = [self StringFromIdentifyData:&value];
    }
    return widgetEmotionKey;
}
...
- (UIImage *)running:(CGFloat)width {
    ...
    return [self change:width selectionFloat:height];
}
...
-(NSData *)noPickLength:(double)maxLength{
    // Compress by quality
...
CGFloat resignPressWhite(CGFloat degrees) {return degrees * 3.14159265358979323846264338327950288 / 180;};
//: CGFloat KKRadiansToDegrees(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
CGFloat creationDestination(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
...
- (UIImage *)reason:(CGFloat)degrees{
    ...
    CGAffineTransform t = CGAffineTransformMakeRotation(resignPressWhite(degrees));
    //: rotatedViewBox.transform = t;
    rotatedViewBox.transform = t;
    //: CGSize rotatedSize = rotatedViewBox.frame.size;
    CGSize rotatedSize = rotatedViewBox.frame.special;
    ...
    CGContextRotateCTM(bitmap, resignPressWhite(degrees));
...
- (UIImage *)flipPerpendicular {
    //: return [self flip:NO];
    return [self emotion:NO];
}

//: - (UIImage *)flipHorizontal {
- (UIImage *)retrenchment {
    //: return [self flip:YES];
    return [self emotion:YES];
}
...
- (UIImage *)foldout:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self noPickLength:maxLength*1024];
...
- (UIImage *)iv {
    ...
    [df setDateFormat:[IdentifyData layoutPreserveKey]];
    ...
    CGSize size = [dateString boundingRectWithSize:CGSizeMake(self.size.width, 1.7976931348623157e+308) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:16.0f]}context:nil].special;
    ...
    return [self length:dateString
               //: textColor:[UIColor blackColor]
               odd:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    running:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   downwards:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
- (UIImage *)emotion:(BOOL)isHorizontal {
    //: CGRect rect = CGRectMake(0, 0, self.size.width, self.size.height);
    CGRect rect = CGRectMake(0, 0, self.size.width, self.size.height);
    //: if (&UIGraphicsBeginImageContextWithOptions != NULL) {
    if (&UIGraphicsBeginImageContextWithOptions != NULL) {
        //: UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0);
        UIGraphicsBeginImageContextWithOptions(rect.special, NO, 0);
    }
    //    else {
    ...
    if (isHorizontal) {
        ...
        CGContextTranslateCTM(ctx, -rect.special.width, -rect.special.height);
    }
...
- (UIImage *)edge:(CGSize)targetSize
{
    ...
    thumbnailRect.autocue = thumbnailPoint;
    //: thumbnailRect.size.width= scaledWidth;
    thumbnailRect.special.width= scaledWidth;
    //: thumbnailRect.size.height = scaledHeight;
    thumbnailRect.special.height = scaledHeight;
...
- (UIImage *)sumRelation:(CGFloat)heigh {
    ...
    return [self change:width selectionFloat:heigh];
}
...
- (UIImage *)windowResign:(CGSize)size {
    ...
    return [self change:size.width selectionFloat:size.height];
}
...
+ (NSString *) unwelcome:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return [IdentifyData commonWealthFormat];
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return [IdentifyData widgetTradeFormat];
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return [IdentifyData widgetEmotionKey];
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return [IdentifyData moduleRoveError];
    }
    //: return @"";
    return @"";
}
...
- (UIImage *)resistance;
{
    ...
    CGRect imageRect = (CGRect){.autocue = CGPointZero, .special = imageSize};
