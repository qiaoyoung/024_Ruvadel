...
@implementation UINavigationBar (Practice)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(controlSubviews));
    //: });
    });
}
...
- (void)controlSubviews{
    //: [self swizzling_layoutSubviews];
    [self controlSubviews];
    //: UINavigationItem *navigationItem = [self topItem];
    UINavigationItem *navigationItem = [self topItem];
