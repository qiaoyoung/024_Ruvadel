...
@interface AgainstDeterData : NSObject

+ (instancetype)sharedInstance;

//: 扩展
@property (nonatomic, copy) NSString *colorDimKey;

//: group_info_activity_op_failed
@property (nonatomic, copy) NSString *featureMagnitudeerfereUtility;

//: %@在群里@了你
@property (nonatomic, copy) NSString *featureArcherSeemPage;

//: ic_more
@property (nonatomic, copy) NSString *screenOperationPlatform;

//: #00B01B
@property (nonatomic, copy) NSString *colorGallSultanTimer;

//: chating_bg
@property (nonatomic, copy) NSString *spacingRetHelper;

//: message_please_enter_content
@property (nonatomic, copy) NSString *coreTenseApeSteadilyMessage;

//: EventName_TapAudio
@property (nonatomic, copy) NSString *moduleEyeValue;

//: back_arrow_bl
@property (nonatomic, copy) NSString *viewLeasedMessage;

//: message_administrator_speak
@property (nonatomic, copy) NSString *themeSteadilyConfig;

//: group_head_def
@property (nonatomic, copy) NSString *layoutCloudFormat;

//: 777777
@property (nonatomic, copy) NSString *moduleFindingMateFlueText;

//: chating_top_bg
@property (nonatomic, copy) NSString *k_gutAlert;

//: 离线
@property (nonatomic, copy) NSString *viewDimPage;

@end

@implementation AgainstDeterData

//: 777777
- (NSString *)moduleFindingMateFlueText {
    if (!_moduleFindingMateFlueText) {
        Byte value[] = {6, 67, 9, 86, 72, 185, 116, 255, 34, 122, 122, 122, 122, 122, 122, 213};
        _moduleFindingMateFlueText = [self StringFromAgainstDeterData:value];
    }
    return _moduleFindingMateFlueText;
}

//: EventName_TapAudio
- (NSString *)moduleEyeValue {
    if (!_moduleEyeValue) {
        Byte value[] = {18, 5, 13, 120, 38, 166, 58, 133, 239, 132, 222, 78, 214, 74, 123, 106, 115, 121, 83, 102, 114, 106, 100, 89, 102, 117, 70, 122, 105, 110, 116, 74};
        _moduleEyeValue = [self StringFromAgainstDeterData:value];
    }
    return _moduleEyeValue;
}

//: 离线
- (NSString *)viewDimPage {
    if (!_viewDimPage) {
        Byte value[] = {6, 14, 11, 74, 14, 170, 126, 27, 144, 36, 250, 245, 180, 201, 245, 200, 205, 50};
        _viewDimPage = [self StringFromAgainstDeterData:value];
    }
    return _viewDimPage;
}

//: #00B01B
- (NSString *)colorGallSultanTimer {
    if (!_colorGallSultanTimer) {
        Byte value[] = {7, 77, 3, 112, 125, 125, 143, 125, 126, 143, 9};
        _colorGallSultanTimer = [self StringFromAgainstDeterData:value];
    }
    return _colorGallSultanTimer;
}

//: group_head_def
- (NSString *)layoutCloudFormat {
    if (!_layoutCloudFormat) {
        Byte value[] = {14, 89, 12, 8, 179, 12, 90, 252, 97, 198, 47, 249, 192, 203, 200, 206, 201, 184, 193, 190, 186, 189, 184, 189, 190, 191, 111};
        _layoutCloudFormat = [self StringFromAgainstDeterData:value];
    }
    return _layoutCloudFormat;
}

//: 扩展
- (NSString *)colorDimKey {
    if (!_colorDimKey) {
        Byte value[] = {6, 54, 5, 110, 49, 28, 191, 223, 27, 231, 203, 41};
        _colorDimKey = [self StringFromAgainstDeterData:value];
    }
    return _colorDimKey;
}

//: back_arrow_bl
- (NSString *)viewLeasedMessage {
    if (!_viewLeasedMessage) {
        Byte value[] = {13, 50, 4, 84, 148, 147, 149, 157, 145, 147, 164, 164, 161, 169, 145, 148, 158, 202};
        _viewLeasedMessage = [self StringFromAgainstDeterData:value];
    }
    return _viewLeasedMessage;
}

//: message_please_enter_content
- (NSString *)coreTenseApeSteadilyMessage {
    if (!_coreTenseApeSteadilyMessage) {
        Byte value[] = {28, 35, 7, 46, 110, 149, 139, 144, 136, 150, 150, 132, 138, 136, 130, 147, 143, 136, 132, 150, 136, 130, 136, 145, 151, 136, 149, 130, 134, 146, 145, 151, 136, 145, 151, 149};
        _coreTenseApeSteadilyMessage = [self StringFromAgainstDeterData:value];
    }
    return _coreTenseApeSteadilyMessage;
}

//: chating_bg
- (NSString *)spacingRetHelper {
    if (!_spacingRetHelper) {
        Byte value[] = {10, 98, 9, 214, 164, 97, 114, 232, 209, 197, 202, 195, 214, 203, 208, 201, 193, 196, 201, 141};
        _spacingRetHelper = [self StringFromAgainstDeterData:value];
    }
    return _spacingRetHelper;
}

//: group_info_activity_op_failed
- (NSString *)featureMagnitudeerfereUtility {
    if (!_featureMagnitudeerfereUtility) {
        Byte value[] = {29, 5, 13, 223, 134, 11, 34, 227, 142, 61, 2, 13, 41, 108, 119, 116, 122, 117, 100, 110, 115, 107, 116, 100, 102, 104, 121, 110, 123, 110, 121, 126, 100, 116, 117, 100, 107, 102, 110, 113, 106, 105, 2};
        _featureMagnitudeerfereUtility = [self StringFromAgainstDeterData:value];
    }
    return _featureMagnitudeerfereUtility;
}

//: %@在群里@了你
- (NSString *)featureArcherSeemPage {
    if (!_featureArcherSeemPage) {
        Byte value[] = {18, 12, 12, 234, 103, 12, 150, 128, 192, 247, 82, 176, 49, 76, 241, 168, 180, 243, 202, 176, 245, 147, 152, 76, 240, 198, 146, 240, 201, 172, 32};
        _featureArcherSeemPage = [self StringFromAgainstDeterData:value];
    }
    return _featureArcherSeemPage;
}

- (NSString *)StringFromAgainstDeterData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self AgainstDeterDataToCache:data]];
}

- (Byte *)AgainstDeterDataToCache:(Byte *)data {
    int millionRecord = data[0];
    Byte samson = data[1];
    int policySurgeryClothes = data[2];
    for (int i = policySurgeryClothes; i < policySurgeryClothes + millionRecord; i++) {
        int value = data[i] - samson;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[policySurgeryClothes + millionRecord] = 0;
    return data + policySurgeryClothes;
}

//: chating_top_bg
- (NSString *)k_gutAlert {
    if (!_k_gutAlert) {
        Byte value[] = {14, 71, 13, 2, 2, 232, 31, 106, 3, 187, 136, 88, 30, 170, 175, 168, 187, 176, 181, 174, 166, 187, 182, 183, 166, 169, 174, 30};
        _k_gutAlert = [self StringFromAgainstDeterData:value];
    }
    return _k_gutAlert;
}

+ (instancetype)sharedInstance {
    static AgainstDeterData *instance = nil;
...
//: message_administrator_speak
- (NSString *)themeSteadilyConfig {
    if (!_themeSteadilyConfig) {
        Byte value[] = {27, 39, 13, 183, 116, 172, 116, 45, 202, 122, 119, 150, 236, 148, 140, 154, 154, 136, 142, 140, 134, 136, 139, 148, 144, 149, 144, 154, 155, 153, 136, 155, 150, 153, 134, 154, 151, 140, 136, 146, 123};
        _themeSteadilyConfig = [self StringFromAgainstDeterData:value];
    }
    return _themeSteadilyConfig;
}

//: ic_more
- (NSString *)screenOperationPlatform {
    if (!_screenOperationPlatform) {
        Byte value[] = {7, 77, 6, 214, 99, 162, 182, 176, 172, 186, 188, 191, 178, 110};
        _screenOperationPlatform = [self StringFromAgainstDeterData:value];
    }
    return _screenOperationPlatform;
}
...
@interface FanViewController ()<NIMMediaManagerDelegate,HistoryDelegate,DelegateGin,NIMConversationManagerDelegate>

//: @property (nonatomic,strong) UIImageView *headerImage;
@property (nonatomic,strong) UIImageView *headerImage;

//: @property (nonatomic,strong) ZZZSessionConfigurator *configurator;
@property (nonatomic,strong) TerrainRare *configurator;
...
@property (nonatomic,strong) ExpertView *badgeView;
...
@property (nonatomic, strong) MpView *audioContent;
...
- (void)deliveryOr
{
    ...
    [self preponderance:self.statuteContentTitle];
    //: [self refreshSessionSubTitle:self.sessionSubTitle];
    [self detect:self.commonName];
...
- (void)fillPoint:(CGFloat)inputHeight
{
    ...
    [self.interactor flyLayout:inputHeight];
}
...
- (MpView *)audioContent
{
    ...
    if(!_audioContent){
        ...
        _audioContent = [[MpView alloc]initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height)];
...
- (void)grace:(InstrumentalityItem *)item
{
    //: [self.interactor mediaPicturePressed];
    [self.interactor cinemaAgreement];
}
...
//    [_sessionInputView endEditing:YES];
    ...
    NIMAudioType type = [self weEven];
    //: NSTimeInterval duration = [AppleProjectKit sharedKit].config.recordMaxDuration;
    NSTimeInterval duration = [Reek style].config.recordMaxDuration;
...
//    self.tableView.backgroundColor = NEEKIT_UIColorFromRGB(0xe4e7ec);
    ...
    [self.tableViewTapGesture addTarget:self action:@selector(plotElementView:)];
...
- (id<NIMConversationManager>)rule{
                 ...
                 productDrawing:(UIView *)view
{
    ...
    if ([self click:message])
    {
        ...
        [self level:message present:view];
    }
    //: handle = YES;
    handle = YES;
    //: return handle;
...
- (void)plotElementView:(id)sender
{
    //: [ZZZBubbleMenuView.shareMenuView removeFromSuperview];
    [UnctionView.fragment removeFromSuperview];
    //: [self.sessionInputView endEditing:YES];
...
- (void)protection:(id)sender
{
    ...
    [self.rule deleteMessage:message];
}
...
- (void)video
{
    ...
    _configurator = [[TerrainRare alloc] init];
    //: [_configurator setup:self];
    [_configurator exceptPreferSetup:self];

    //: BOOL needProximityMonitor = [self needProximityMonitor];
    BOOL needProximityMonitor = [self beam];
...
- (void)level:(NIMMessage *)message present:(UIView *)view {

    //: [[ZZZBubbleMenuView shareMenuView] removeFromSuperview];
    [[UnctionView fragment] removeFromSuperview];

    //: NSArray *btns = [self genMediaButtonsWithMessage:message];
    NSArray *btns = [self sequence:message];
    ...
    if (btns.count > 0) {
        ...
        [[UnctionView fragment] searchionOdd:btns with:cursorStartRectToWindow lat:tempRect richPersonRange:^(InstrumentalityItem *item) {
            @
...
- (void)onRecvQuickComment:(NIMQuickComment *)comment
{
    ...
    [self.interactor force:comment.message];
}
...
- (void)viewDidLoad {
    ...
    //    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"chating_bg"]];
        ...
        bg.image = [UIImage imageNamed:[AgainstDeterData sharedInstance].spacingRetHelper];
...
- (void)wait
{
    ...
    if ([self mobileView])
    {
        ...
        [self.sessionInputView setStanding:self];
...
- (void)single
{
    ...
    _topview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice be]))];
//    _topview.backgroundColor = [UIColor colorWithRed:216/255.0 green:242/255.0 blue:253/255.0 alpha:1];
    ...
    bg.image = [UIImage imageNamed:[AgainstDeterData sharedInstance].k_gutAlert];
    ...
    [backBtn setImage:[UIImage imageNamed:[AgainstDeterData sharedInstance].viewLeasedMessage] forState:UIControlStateNormal];
    //: [_topview addSubview:backBtn];
    [_topview addSubview:backBtn];

//    NSString *userID = self.session.sessionId;
//    NIMUser *me = [[NIMSDK sharedSDK].userManager userInfo:userID];
//    RobInvite *info = [[Reek sharedKit] infoByUser:userID option:nil];

    //: self.sessionUnreadCount = [NIMSDK sharedSDK].conversationManager.allUnreadCount;
    self.sessionUnreadCount = [NIMSDK sharedSDK].conversationManager.allUnreadCount;

     //: _badgeView = [ZZZBadgeView viewWithBadgeTip:@""];
     _badgeView = [ExpertView collectRequest:@""];
...
//    [_headerImage sd_setImageWithURL:[NSURL URLWithString:me.userInfo.avatarUrl] placeholderImage:info.avatarImage];
    ...
    _labtitle = [[UILabel alloc]initWithFrame:CGRectMake(backBtn.right+10, [UIDevice be], 200, 40)];
    //: _labtitle.textColor = [UIColor blackColor];
    _labtitle.textColor = [UIColor blackColor];
    //: _labtitle.font = [UIFont boldSystemFontOfSize:16];
    _labtitle.font = [UIFont boldSystemFontOfSize:16];
    //: _labtitle.text = self.sessionTitle;
    _labtitle.text = self.statuteContentTitle;
...
//        [enterTeamCard setImage:[UIImage imageNamed:@"ic_more"] forState:UIControlStateNormal];
        ...
        [enterTeamCard sd_setImageWithURL:[NSURL URLWithString:team.avatarUrl] forState:(UIControlState)UIControlStateNormal placeholderImage:[UIImage imageNamed:[AgainstDeterData sharedInstance].layoutCloudFormat]];
        //: enterTeamCard.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-width-15, [UIDevice vg_statusBarHeight]+10, width, width);
        enterTeamCard.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-width-15, [UIDevice be]+10, width, width);
...
//        [_topview addSubview:btnVoice];
        ...
        [infoBtn setImage:[UIImage imageNamed:[AgainstDeterData sharedInstance].screenOperationPlatform] forState:UIControlStateNormal];
        //: infoBtn.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-width-15, [UIDevice vg_statusBarHeight]+10, width, width);
        infoBtn.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-width-15, [UIDevice be]+10, width, width);
        ...
        [coordinator animateAlongsideTransition:^(id <UIViewControllerTransitionCoordinatorContext> context)
         {
             ...
             [wself.interactor roll];
             //: [wself.sessionInputView reset];
             [wself.sessionInputView measure];
...
- (void)stamp:(NIMRecentSession *)recentSession
         //: totalUnreadCount:(NSInteger)totalUnreadCount{
         render:(NSInteger)totalUnreadCount{
        ...
        if( message.messageType == NIMMessageTypeImage){
            ...
            if(data && urlString){
                ...
                [[SDImageCache sharedImageCache] storeImageData:data forKey:key completion:^{
                    ...
                    [weakSelf.interactor force:message];
                //: }];
                }];
            }
        //: }else{
...
//            [self.tableView reloadData];
            ...
            message.session.sessionType == NIMSessionTypeSuperTeam)
        {
            ...
            [self.interactor display:@[receipt]];
        }
    }
...
- (void)setRemove:(NSArray <NIMMessage *>*)messages {
...
- (NSString *)statuteContentTitle
{
    ...
    switch (type) {
        ...
        case NIMSessionTypeP2P:{
            ...
            title = [PayerMinimal anyBlue:self.session.sessionId detail:self.session];
        }
            //: break;
            break;
        //: case NIMSessionTypeSuperTeam: {
...
#pragma mark - ScopeBoa
//: - (BOOL)onTapMediaItem:(ZZZMediaItem *)item{
- (BOOL)reported:(InstrumentalityItem *)item{
...
- (void)scaleHolder:(NIMMessage *)message
{
    ...
    [self.interactor prefer:message from:nil];
...
- (void)blunt:(InstrumentalityItem *)item
{
//    [self.interactor mediaLocationPressed];
    ...
    [self.audioContent load];

}


//: - (void)scrollViewDidScroll:(UIScrollView *)scrollView {
- (void)numberIcon:(UIScrollView *)scrollView {
...
- (void)setScene:(BOOL)ban {
    ...
    if (ban){
        ...
        [self.sessionInputView kit:[CarefulRage formatExtend:[AgainstDeterData sharedInstance].themeSteadilyConfig] standard:[UIColor redColor]];
    ...
    //: } else {
        ...
        [self.sessionInputView kit:[CarefulRage formatExtend:[AgainstDeterData sharedInstance].coreTenseApeSteadilyMessage] standard:[UIColor grayColor]];
...
- (void)replacement:(DepictEmoticon *)emoticon
{
    ...
    __block NIMQuickComment *newComment = [YardPrefer underFormat:number listenerInExt:emoticon.tag policy:[AgainstDeterData sharedInstance].colorDimKey];

    //: __weak typeof(self) weakSelf = self;
    __weak typeof(self) weakSelf = self;
    //: [self hadCommentThisMessage:self.messageForMenu type:number
    [self nameUp:self.messageForMenu complete:number
                      //: compltion:^(NSMapTable *result)
                      perform:^(NSMapTable *result)
     {
        //: NIMQuickComment *oldComment = [ZZZKitQuickCommentUtil myCommentFromComments:0 keys:@[@(number)] comments:result];
        NIMQuickComment *oldComment = [EyUtil zoneComments:0 candid:@[@(number)] element:result];
        ...
        if (!contains)
        {
            ...
            [weakSelf.interactor m:newComment
                                  //: completion:^(NSError *error)
                                  soapBubble:^(NSError *error)
            {
//                [self.view hideToasts];
                ...
                if (error)
                {
                    ...
                    [weakSelf.view res:[CarefulRage formatExtend:[AgainstDeterData sharedInstance].featureMagnitudeerfereUtility] enableQuick:2 primary:themeForceTitle];
                }

                //: [weakSelf cleanMenuMessage];
                [weakSelf adductMessage];
            //: }];
            }];
        }
        //: else
        else
        {
            ...
            [weakSelf.interactor targetMessageCompletion:oldComment
                                   //: targetMessage:weakSelf.messageForMenu
                                   factor:weakSelf.messageForMenu
                                      //: completion:^(NSError *error)
                                      related:^(NSError *error)
            {
//                [self.view hideToasts];
                //: if (error)
                if (error)
...
- (void)nameUp:(NIMMessage *)message
                         //: type:(int64_t)type
                         complete:(int64_t)type
                    //: compltion:(void(^)(NSMapTable *))completion
                    perform:(void(^)(NSMapTable *))completion
{
...
- (void)add:(NSArray *)messages
{
    ...
    [self.interactor challenge:messages];
}


//: - (void)onTapMediaItemShoot:(ZZZMediaItem *)item
- (void)memory:(InstrumentalityItem *)item
{
    //: [self.interactor mediaShootPressed];
    [self.interactor instance];
}
...
- (void)excessSafetyOrigin:(NSString *)text portUsers:(NSArray *)atUsers
{
    ...
    NIMMessage *message = [MessageMaker top:text];
    //: if (atUsers.count)
    if (atUsers.count)
    {
        ...
        NSString *me = [[Reek style].provider sequence:[NIMSDK sharedSDK].loginManager.currentAccount from:option].showName;
        //: apnsOption.apnsContent = [NSString stringWithFormat:@"%@在群里@了你".string_localized, me];
        apnsOption.apnsContent = [NSString stringWithFormat:[AgainstDeterData sharedInstance].featureArcherSeemPage.title, me];
        //: message.apnsMemberOption = apnsOption;
        message.apnsMemberOption = apnsOption;
    }

    //: [self sendMessage:message];
    [self scaleHolder:message];
}

//获取当前最上层的控制器
//: - (UIViewController *)getTopMostController {
- (UIViewController *)representation {
...
- (void)detect:(NSString *)title
{
    ...
    if([title isEqualToString:[AgainstDeterData sharedInstance].viewDimPage.maximal]){
        //: self.subtitle.textColor = [UIColor colorWithHexString:@"777777"];
        self.subtitle.textColor = [UIColor port:[AgainstDeterData sharedInstance].moduleFindingMateFlueText];
    //: }else{
    }else{
        //: self.subtitle.textColor = [UIColor colorWithHexString:@"#00B01B"];
        self.subtitle.textColor = [UIColor port:[AgainstDeterData sharedInstance].colorGallSultanTimer];
    }
    //: [self setUpTitleView];
    [self always];
}
...
- (void)onRecvMessages:(NSArray *)messages
{
    ...
    if ([self abstractMsg])
    {
        ...
        [self representation:messages];
        //: [self.interactor markRead:NO];
        [self.interactor jump:NO];

        // 群禁言后输入框改变占位符文字
        //: [self setBannedSpeakingWithMessage:messages];
        [self setRemove:messages];
    }
}
...
- (void)always
{
    ...
    BackgroundView *titleView = (BackgroundView *)self.navigationItem.titleView;
    //: if (!titleView || ![titleView isKindOfClass:[ZZZKitTitleView class]])
    if (!titleView || ![titleView isKindOfClass:[BackgroundView class]])
    {
        //: titleView = [[ZZZKitTitleView alloc] initWithFrame:CGRectZero];
        titleView = [[BackgroundView alloc] initWithFrame:CGRectZero];
        //: self.navigationItem.titleView = titleView;
        self.navigationItem.titleView = titleView;

        //: titleView.titleLabel.text = self.sessionTitle;
        titleView.titleLabel.text = self.statuteContentTitle;
        //: titleView.subtitleLabel.text = self.sessionSubTitle;
        titleView.subtitleLabel.text = self.commonName;
    ...
    self.subtitle.text = self.commonName;
}
...
#pragma mark - HoSumeractDelegate
...
- (BOOL)onlied:(HillInside *)event{
    ...
    if ([eventName isEqualToString:[AgainstDeterData sharedInstance].moduleEyeValue])
    {
        //: [self.interactor mediaAudioPressed:event.messageModel];
        [self.interactor aspectAcross:event.messageModel];
        //: handle = YES;
        handle = YES;
    }
    //: return handle;
...
- (void)messagesDeletedInSession:(NIMSession *)session{
    ...
    [self.interactor conversation:nil];
    //: [self.tableView reloadData];
    [self.tableView reloadData];
}
...
- (void)window:(NSArray<NIMMessageReceipt *> *)receipts
{
    //: [self.interactor checkReceipts:receipts];
    [self.interactor display:receipts];
}
...
- (void)sureCatalog:(NSString *)chartletId
                 //: catalog:(NSString *)catalogId
                 pauseQuality:(NSString *)catalogId
{
...
//
//    NIMMessage *message = [MessageMaker msgWithImage:gif];
//    [self sendMessage:message];
//

    //: NIMMessage *message = [ZZZMessageMaker msgWithImageData:imageData extension:@""];
    NIMMessage *message = [MessageMaker trackExtension:imageData external:@""];
...
//    NSData *imageData = UIImagePNGRepresentation(image);
    ...
    [self.sessionInputView.toolBar setResult:[CarefulRage formatExtend:[AgainstDeterData sharedInstance].coreTenseApeSteadilyMessage]];
               ...
               legal:(BOOL)isSelected
{
    ...
    if (isSelected)
    {
        ...
        [self.interactor targetMessageCompletion:comment
...
//            [self.view hideToasts];
    ...
    else
    {
        ...
        [self.interactor exclusiveCompletion:aComment
                               //: toMessage:message
                               color:message
                              //: completion:^(NSError *error)
                              fill:^(NSError *error)
...
- (GraftModel *)hangUpMessage:(NIMMessage *)message{
    ...
    GraftModel *model = [self.interactor removeMessage:message];
...
- (void)fetchMessageAttachment:(NIMMessage *)message didCompleteWithError:(NSError *)error
{
    ...
    if ([message.session isEqual:_session])
    {
        ...
        GraftModel *model = [self.interactor icon:message];
...
- (NSString *)commonName{return @"";}


//: - (void)copyText:(id)sender
- (void)ovalDestination:(id)sender
{
    //: NIMMessage *message = [self messageForMenu];
    NIMMessage *message = [self messageForMenu];
    //: if (message.tyl_internalIdentifier.length) {
...
- (NSArray *)sequence:(NIMMessage *)message {
...
- (void)disturbing
{
    ...
    if(self.sessionUnreadCount>0){
        ...
        _labtitle.frame = CGRectMake(_badgeView.right+10, [UIDevice be], 100, 40);
    }
    //: else{
    else{
        ...
        _labtitle.frame = CGRectMake(65, [UIDevice be], 200, 40);
    }
}
...
- (void)recordAudio:(NSString *)filePath didCompletedWithError:(NSError *)error {
    ...
    if(!error) {
        ...
        if ([self sub:filePath]) {
            //: [self sendMessage:[ZZZMessageMaker msgWithAudio:filePath]];
            [self scaleHolder:[MessageMaker security:filePath]];
        //: }else{
        }else{
