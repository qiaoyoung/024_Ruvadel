...
@implementation PrecisTool

//: + (void)requestPhotoLibraryAuthorization:(void(^)(EnumAuthorizationStatus status))callback
+ (void)randomAuthorization:(void(^)(EnumAuthorizationStatus status))callback
{
        ...
        if (authStatus == ALAuthorizationStatusNotDetermined) { // 未授权
            ...
            [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
                ...
                if (status == PHAuthorizationStatusAuthorized) {
                    ...
                    [self push:callback yet:EnumAuthorizationStatusAuthorized];
                //: } else if (status == PHAuthorizationStatusDenied) {
                } else if (status == PHAuthorizationStatusDenied) {
                    //: [self executeCallback:callback status:EnumAuthorizationStatusDenied];
                    [self push:callback yet:EnumAuthorizationStatusDenied];
                //: } else if (status == PHAuthorizationStatusRestricted) {
                } else if (status == PHAuthorizationStatusRestricted) {
                    //: [self executeCallback:callback status:EnumAuthorizationStatusRestricted];
                    [self push:callback yet:EnumAuthorizationStatusRestricted];
                }
            //: }];
            }];
        }
    ...
    //: } else {
        ...
        [self push:callback yet:EnumAuthorizationStatusNotSupport];
    }
}

//: + (void)requestCameraAuthorization:(void(^)(EnumAuthorizationStatus status))callback
+ (void)seeYear:(void(^)(EnumAuthorizationStatus status))callback
{
...
+ (void)push:(void (^)(EnumAuthorizationStatus))callback yet:(EnumAuthorizationStatus)status {
...
+ (void)inputDismissAuthorization:(void (^)(EnumAuthorizationStatus))callback
{
