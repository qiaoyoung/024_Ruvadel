...
@interface UmberData : NSObject

+ (instancetype)sharedInstance;

//: arrowUp
@property (nonatomic, copy) NSString *moduleRecMessage;

//: arrowDown
@property (nonatomic, copy) NSString *moduleGalleryEvent;

@end

@implementation UmberData

+ (instancetype)sharedInstance {
    static UmberData *instance = nil;
...
- (NSString *)StringFromUmberData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self UmberDataToCache:data]];
}

- (Byte *)UmberDataToCache:(Byte *)data {
    int discoTurn = data[0];
    Byte purchase = data[1];
    int insightTun = data[2];
    for (int i = insightTun; i < insightTun + discoTurn; i++) {
        int value = data[i] + purchase;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[insightTun + discoTurn] = 0;
    return data + insightTun;
}

//: arrowDown
- (NSString *)moduleGalleryEvent {
    if (!_moduleGalleryEvent) {
		NSArray<NSNumber *> *origin = @[@9, @83, @6, @74, @87, @224, @14, @31, @31, @28, @36, @241, @28, @36, @27, @238];
		NSData *data = [UmberData UmberDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _moduleGalleryEvent = [self StringFromUmberData:value];
    }
    return _moduleGalleryEvent;
}

+ (NSData *)UmberDataToData:(NSArray<NSNumber *> *)value {
...
//: arrowUp
- (NSString *)moduleRecMessage {
    if (!_moduleRecMessage) {
		NSArray<NSNumber *> *origin = @[@7, @45, @9, @86, @229, @221, @76, @153, @67, @52, @69, @69, @66, @74, @40, @67, @157];
		NSData *data = [UmberData UmberDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _moduleRecMessage = [self StringFromUmberData:value];
    }
    return _moduleRecMessage;
}
...
//        CGFloat y = topMargin + i / countInOneLine * (buttonHeight + topMargin * 2);
        ...
        [button addTarget:self action:@selector(housed:) forControlEvents:UIControlEventTouchUpInside];

        //: ZZZBubbleButtonModel *model = array[i];
        ExpertModel *model = array[i];
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UITapGestureRecognizer *singleTapbox1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(oddMode)];
...
- (BOOL)quantityeraction:(NSArray *)newArray {
    ...
    for (int i = 0; i < newArray.count; i ++) {
        ...
        NSInteger newId = ((ExpertModel *)_oldContentArray[i]).buttonId;
        //: for (int j = 0; j < _oldContentArray.count; j ++) {
        for (int j = 0; j < _oldContentArray.count; j ++) {
            //: NSInteger oldId = ((ZZZBubbleButtonModel *)_oldContentArray[j]).buttonId;
            NSInteger oldId = ((ExpertModel *)_oldContentArray[j]).buttonId;
            //: if (newId == oldId) {
            if (newId == oldId) {
                //: finded = true;
                finded = true;
...
- (void)housed:(UIButton *)button {
    ...
    if (_myNewContentArray.count > button.tag - 100) {
        ...
        ExpertModel *model = (ExpertModel *)_myNewContentArray[button.tag - 100];
...
- (void)searchionOdd:(NSArray *)array
                 //: cursorStartRect:(CGRect)cursorStartRect selectionTextRectInWindow:(CGRect)rect selectBlock:(void(^)(ZZZMediaItem *data))block {
                 with:(CGRect)cursorStartRect lat:(CGRect)rect richPersonRange:(void(^)(InstrumentalityItem *data))block {
    //: _selectBlock = block;
    _selectBlock = block;
//    ([UIApplication sharedApplication].delegate).window.backgroundColor = UIColor.redColor;
    ...
    UIView *buttonsBgView = [self buttons:array];
    ...
    if (direction == ZZZBubbleMenuViewDirectionPriorityType_Up) {
        ...
        _arrowView.image = [UIImage imageNamed:[UmberData sharedInstance].moduleGalleryEvent];
    //: } else {
    } else {
        //: arrowRect.origin.x = rect.origin.x + rect.size.width / 2.0 - selfRect.origin.x - _arrowWidth / 2.0;
        arrowRect.origin.x = rect.origin.x + rect.size.width / 2.0 - selfRect.origin.x - _arrowWidth / 2.0;
        ...
        _arrowView.image = [UIImage imageNamed:[UmberData sharedInstance].moduleRecMessage];
    }
...
+ (instancetype)fragment {
    //: static ZZZBubbleMenuView *menu = nil;
    static UnctionView *menu = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
