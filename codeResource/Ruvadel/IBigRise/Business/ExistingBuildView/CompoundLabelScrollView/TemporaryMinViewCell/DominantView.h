...
@protocol DownwardDelegate <NSObject>
...
- (void)prixFixeClientQuit:(DominantView *)menuView savingGrace:(TemporaryMinViewCell *)menuCell overkill:(NSInteger)index;

//: - (void)ffDropDownMenuViewWillAppear;
- (void)strengthAdjustment;
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage StrippedDownBasedModel instances
 ...
 *               若用框架自带的cell,直接传@"BriefViewCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"BriefViewCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of TemporaryMinViewCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"BriefViewCell"
...
@property (nonatomic, assign) CGFloat boundaryFlip;
...
@property (nonatomic, weak) id<DownwardDelegate> delegate;
 ...
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage StrippedDownBasedModel instances or the StrippedDownBasedModel's subclass instances )
 */
 ...
 */
//: @property (nonatomic, assign) CGSize iconSize;
@property (nonatomic, assign) CGSize organizeSize;
 ...
 */
//: @property (nonatomic, assign) CGFloat triangleY;
@property (nonatomic, assign) CGFloat nomadic;
...
+ (instancetype)defaultInsideShoreline:(NSArray *)menuModelsArray end:(CGFloat)menuWidth activity:(CGFloat)eachItemHeight beforeBy:(CGFloat)menuRightMargin already:(CGFloat)triangleRightMargin;
