...
- (void)layoutSubviews {
    ...
    for (NSUInteger i = 0; i < self.dataSource.count; i++) {
        ...
        pwdTextField.frame = CGRectMake(x, y, w, h);
	[self setGazette:_elementBorderColor];
    }
}
...
- (void)white {
    ...
    self.textField.text = nil;
	[self setGasPointKeyboard:_autoHideKeyboard];
    //: [self textChange:self.textField];
    [self secureReceiver:self.textField];
}

- (BOOL)protection:(BOOL)gasPointKeyboard {
    //: OC_CUSTOM_PROPERTY_INJECT
    _gasPointKeyboard = gasPointKeyboard;
    return gasPointKeyboard;
}
...
- (void)setElementCount:(NSInteger)elementCount {
	...
	[self setGasPointKeyboard:_autoHideKeyboard];
        ...
        pwdTextField.textAlignment = NSTextAlignmentCenter;
	[self setGasPointKeyboard:_autoHideKeyboard];//居中
...
- (UIColor *)extraInstance:(UIColor *)gazette {
    //: OC_CUSTOM_PROPERTY_INJECT
    _gazette = gazette;
    return gazette;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(context, [self extraInstance:self.elementBorderColor].CGColor);
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    //: [self showKeyboard];
    [self versionKeyboard];
}

- (void)setGasPointKeyboard:(BOOL)gasPointKeyboard {
    //: OC_CUSTOM_PROPERTY_INJECT
    _gasPointKeyboard = gasPointKeyboard;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
	...
	[self setGazette:_elementBorderColor];
        ...
        [textField addTarget:self action:@selector(secureReceiver:) forControlEvents:UIControlEventEditingChanged];
...
- (void)secureReceiver:(UITextField *)textField {
    ...
    if (password.length == self.dataSource.count)
    {
        ...
        if ([self protection:self.autoHideKeyboard]) {
            //: [self hideKeyboard];
            [self expected];//隐藏键盘
        }
    }
...
- (void)setGazette:(UIColor *)gazette {
    //: OC_CUSTOM_PROPERTY_INJECT
    _gazette = gazette;
}
