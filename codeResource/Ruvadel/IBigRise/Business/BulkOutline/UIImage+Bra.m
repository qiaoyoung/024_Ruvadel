...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(refuses:));
...
+ (UIImage *)refuses:(NSString *)name {
    ...
    UIImage *image = [[BulkOutline playCreation] refuses:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self refuses:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
